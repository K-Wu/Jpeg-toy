...
for L=3:5
	face_vect=train_detection(L,dev_index,0);%计算人脸向量
	num_dev=0;
	dists=zeros(1,0);
	for img_ind =1:33
	...
	L%打印L
	dists%打印距离矩阵以及最大距离
	max_dist=max(dists);%这个值作为阈值，在L=4和5时表现不好
	max_dist=0.002%钦定阈值为0.068
	precision=0;
	TEST_NUM=10;
	CASE_PER_TEST=5;
	...
	for curr_test=1:TEST_NUM
		...
		face_vect=train_detection(L,test_case,0);
		num_correct=0;%正确的数量
		for img_ind =1:33
		...
		precision=precision+num_correct;
	end
	nums_correct
	precision=precision/TEST_NUM/CASE_PER_TEST%几次测试平均准确率
end